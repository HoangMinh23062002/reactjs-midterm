...
class List extends Component {
  ...
  componentDidMount() {
    ...
    if (match) {
      ...
      axios({
      method: 'GET',
      url :`https://61c189b59dbcca0017c81f4a.mockapi.io/api/cart/${id}`,
      data : null
     }).then(res =>{
      var data =res.data;
        this.setState({
    ...
    axios.get('https://61c189b59dbcca0017c81f4a.mockapi.io/api/cart').then(res => {
  ...
  onSave = (event) => {
    ...
    if (this.state.id ==''){
      if(this.state.name !== '' && this.state.price!=='' && this.state.quantity !== '' && this.state.image!==''){
        axios({
          method:'POST',
          url :`https://61c189b59dbcca0017c81f4a.mockapi.io/api/cart`,
          data: {
      ...
      axios({
        method:'PUT',
        url :`https://61c189b59dbcca0017c81f4a.mockapi.io/api/cart/${this.state.id}`,
        data: {
          name: this.state.name,
          price: this.state.price,
          oldPrice : this.state.oldPrice,
    ...
    this.state.products.map((product) =>(
      ...
      <div className="card col-4" style={{ width: "18rem" }}>
        ...
        <h5 className="card-title">{product.name}</h5>
          ...
          <button className=" action-button" role="button" href="#" onClick={() =>this.showEditProduct(product.id)}>
            Edit
          </button>
          <button className="" onClick={() =>this.onDelete(product.id)}>
            delete
          </button>
